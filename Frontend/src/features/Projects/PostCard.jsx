...
function PostCard({ pid, sx, data, ...rest }) {
  ...
  const dateString = data?.created_at
    ? format(new Date(data.created_at), "do MMM y | h:mmaaa")
    : "7th May 2022 | 9:03pm";
  const avatarUrl = project?.avatar_url ?? "";
  const body =
    data.body ??
    "Good day to all! This is to announce our workshop happening on May 14th. Please come if you want to learn Node.js";
  ...
  return (
    <Card {...rest} sx={{ width: "100%", ...sx }}>
      <CardHeader
        avatar={<LinkableAvatar src={avatarUrl} />}
        title={title}
        subheader={<p style={{ margin: 0 }}>{dateString}</p>}
      ></CardHeader>

      <CardContent sx={{ paddingTop: 0, paddingBottom: 0 }}>
        <Tag color="var(--tag-grey)" variant="body2" sx={{ marginBottom: 3 }}>
          {isPoll ? "Poll" : "Post"}
        </Tag>
